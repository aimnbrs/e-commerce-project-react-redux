...
const productCollection = () => async (dispatch) => {
...
const creatProduct = (category, model, price, url) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({ type: productConst.CREATPRODUCT_FAILURE, payload: error.message });
    console.log("error",store.getState());
  }
};
...
const updateProduct = (id) => async (dispatch) => {
    ...
    try {
      ...
      dispatch({ type: productConst.UPDATEPRODUCT_FAILURE, payload: error.message });
      console.log("error",store.getState());
    }
  };
